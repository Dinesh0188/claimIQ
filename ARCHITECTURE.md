...
- **Auth is off by default, not absent.** Authentication and tenancy exist and are
  tested, but only activate when configured — `CLAIMIQ_API_KEYS` enables API-key auth
  with per-tenant scopes. The audit ledger is separate: it is on by default and records
  every determination, as `key_id="anonymous"` under tenant `"local"` until keys are set.
  Exposing it publicly with real claim data requires the operator to enable auth first; a
  public bind with no keys is flagged `insecure_deployment`.
