...
- **Authentication, tenancy and the audit ledger exist and are tested, but are off by
  default.** The demo ships bound to `127.0.0.1` with no keys, so a clone works
  unconfigured. Setting `CLAIMIQ_API_KEYS` (`key:tenant:scope` entries, constant-time
  comparison) turns on API-key auth with per-tenant scope enforcement; the append-only
  ledger records who asked for each determination. A public bind with no keys is reported
